...
var game = { on : false,
	refresh : { all : function() {
		...
		if (endless.w) {
			if (side == 'green') {
				...
				if (endless.t > ls.sc) {
					...
					endless.t = 1;
					endless.k = 0;
					ga("send", "event", "battle", "lost-endless-battle", "release");
					return;
				}
		...
		if (side == 'green') {
			...
			ga("send", "event", "battle", "won-battle", "release");
		} else {
			...
			$('overlay').style.backgroundColor = '#b30005';
			injectStyles('#restartText { background: #960000 } #restartText:hover { background: #a00000 }');
			ga("send", "event", "battle", "lost-battle", "release");
		}
		$('restartText').style.display = "none";
		setTimeout(function(){$('restartText').style.display = "block"}, 750);
		$('overlay').style.display = "block";
		...
		if (game.on == true) {
			if (side == 'green') {
				...
				b.health = Math.min(b.orig_health, b.health);
				heals++;
				if (heals == 10) ga("send", "event", "battle", "tenth-heal", "release");
			} else if (side == 'red') {
...
$('bSection').addEventListener('touchend', function(){game.heal('green')});
$('aSection').addEventListener('touchend', function(){
...
$('restartText').addEventListener('touchend', function(){restart()});
...
document.addEventListener('keyup', function(e){
	keypresses++;
	if (keypresses == 1) ga("send", "event", "battle", "first-keypress", "release");
	...
	if ((k == 46 || k == 32) && game.on == false) {
		ga("send", "event", "battle", "restarted-battle", "release");
		restart();
	}
