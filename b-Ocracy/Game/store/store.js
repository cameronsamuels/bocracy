...
function refresh(boxToSelect) {
	...
	for (i = 0; i < goodNames[battleground].length; i++) {
		...
		e.addEventListener("touchend", select);
...
function upgrade(e) {
	...
	if (ls.redbacks >= costToUpgrade) {
		...
		ga("send", "event", "store", "upgraded-character", "release");
	}
}
...
document.querySelector("section:first-child>div:nth-child(2)>div:nth-child(2)>div:nth-child(2)").addEventListener("touchend", function() {
	...
	if (ls.redbacks >= cost && ls[character] != "true") {
		...
		ls[character] = true;
		// Trigger event to track in Google Analytics
		ga("send", "event", "store", "unlocked-character", "release");
		// Refresh the layout
		refresh(character);
	}
});
...
for (i = 0; i < 3; i++) {
	...
	document.querySelectorAll("section:first-child>div:nth-child(2)>div:first-child>div")[i].addEventListener("touchend", upgrade);
}
// Add a back button click event
document.querySelector("main>img").addEventListener("touchend", function(){ location = "../index.html" });
// Add a change to previous battleground click event
document.querySelector("section:nth-child(2) img:nth-child(4)").addEventListener("touchend", function(){ changeBattleground(-1) });
// Add a change to next battleground click event
document.querySelector("section:nth-child(2) img:nth-child(5)").addEventListener("touchend", function(){ changeBattleground(1) });
